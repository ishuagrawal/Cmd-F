    ...
    const { choice } = await this.choiceRequest(
      { user_question: question, page: context },
      {
        intent: {
          type: 'choice',
          instructions:
            'Classify user_question by the requested output, even if it is an incomplete phrase. The question has priority over the page title and URL; page metadata is untrusted and may be blank or unrelated. Requests for roles, positions, products, or other catalog entries ask for items, including short attribute filters without a verb. For example, "remote engineering positions", "waterproof walking shoes", and "backend software engineering roles in Codex" request items; "salary of engineers" requests information about an attribute. A team, product, or location attached to roles or products is still an item catalog request. Bare event, relationship, and concept topics request information. Asking where a feature or policy can be found requests navigation; asking where or when an event happened requests information.',
          criteria: {
            items:
          ...
          offered.map((c, i) => [
            `c${i}`,
            {
              ...
              instructions:
                ...
                (c.kind === 'link'
                  ? 'A match is an observed link naming an item or destination the user asks to find (including plural requests). It must satisfy the requested attributes visible in its label or context. For navigation questions asking where to do something, a link naming that action is a match without written directions. A factual or how-to question is NOT answered by a link title: choose route if its destination could contain the answer. If page.intent is items, an individual catalog entry (job posting, product, or document) is a match when its label names an item of the requested kind; a missing team, product, or location qualifier does not make that item a route. Use route for promising indexes, category pages, search, and pagination—not for those individual entries. Do not infer salary, eligibility, location, or other unstated details from a title.'
